...
public class EnvironmentVariableProviderTests
{
    ...
    public async Task GetValueAsync_ReturnsValue_WhenEnvVarExists()
    {
        ...
        Assert.True(result.Value.TryGetProperty(key, out var prop));
    ...
    public async Task GetValueAsync_WithPrefix_ReturnsOnlyPrefixedVars()
    {
        ...
        // Assert
        Assert.NotNull(result);
        Assert.True(result.Value.TryGetProperty("SETTING1", out var v1));
        Assert.True(result.Value.TryGetProperty("SETTING2", out var v2));
        Assert.False(result.Value.TryGetProperty("SETTING", out _));
    ...
    public async Task Changes_ReturnsEmptyObservable()
    {
       ...
       Assert.Equal(configChangeNotification.Part, "Environment");
    }
}
