...
public class ConfigManagerHealthCancellationTests
{
    ...
    {
        // Arrange – one slow provider rule that we can trigger changes on.
        var rule = new ConfigRule(
            ...
            options: new ConfigRuleOptions(Required: true)
        );
    ...
    using var sub = healthService.SnapshotStream.Subscribe(h => healthEmissions.Add(h));
        ...
        // Act: Trigger two rapid changes. The second should cancel the first fetch.
        ...
        // Assert: At most one new emission (successful recompute). Cancellation should not emit.
