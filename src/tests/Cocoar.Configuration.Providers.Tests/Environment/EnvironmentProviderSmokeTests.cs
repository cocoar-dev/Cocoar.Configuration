...
/// <summary>
/// EnvironmentProviderUnitTests
/// -----------------------------
/// PURPOSE
///   Environment variable provider tests with deterministic snapshot semantics.
///   Validates prefix handling, delimiter parsing, hierarchy creation, and edge cases
///   for environment variable configuration sources.
/// 
/// SCOPE
///   - Basic environment variable reading and binding
///   - Prefix filtering and trimming behavior
///   - Delimiter handling (single underscore, double underscore, colon)
///   - Hierarchy creation from delimited keys
///   - Edge cases (missing variables, empty prefixes, non-standard separators)
///   - Health degradation behavior for missing required variables
/// 
/// COVERAGE
///   - Variable presence and absence handling
///   - Scalar value binding to configuration objects
///   - Nested hierarchy mapping via multiple delimiter types
///   - Prefix edge cases (non-delimiter characters, underscore trimming)
///   - Complex delimiter combinations in single test scenarios
///   - Global variable loading (empty prefix scenarios)
/// 
/// PATTERNS
///   Uses EnvScope.Set() for deterministic environment variable testing
///   with automatic cleanup and isolation between test runs.
/// </summary>
// Phase 1 Environment provider unit tests (deterministic snapshot semantics)
public class EnvironmentProviderUnitTests
{
    ...
    public void MissingExpectedVariable_DoesNotDegradeRule()
    {
        ...
        var rule = Rule.From.Environment(prefix).Required().For<object>().Build();
    ...
    public void RequiredVariablePresent_BindsScalar_Healthy()
    {
        ...
        var rule = Rule.From.Environment(prefix).Required().For<SimpleValueConfig>().Build();
        using var manager = new ConfigManager(new[]{rule});
        manager.Initialize();
    ...
    public void NestedMapping_DoubleUnderscore_SplitsIntoHierarchy()
    {
        ...
        var rule = Rule.From.Environment(prefix).Required().For<AppSettings>().Build();
        using var manager = new ConfigManager(new[]{rule});
        manager.Initialize();
    ...
    public void EmptyPrefix_LoadsAllVariables()
    {
        ...
        var rule = Rule.From.Environment().Required().For<Dictionary<string, object>>().Build();
        using var manager = new ConfigManager(new[]{rule});
        manager.Initialize();
    ...
    public void NonDelimiterPrefixCharacter_Works()
    {
        ...
        var rule = Rule.From.Environment(prefix).Required().For<Dictionary<string, object>>().Build();
        using var manager = new ConfigManager(new[]{rule});
        manager.Initialize();
