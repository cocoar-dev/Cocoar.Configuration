...
public class FileProviderUnitTests
{
    ...
    public void MissingFile_OptionalRule_SkipsHealthy()
    {
        ...
        var rule = Rule.From.File(path).For<object>().Required(false).Build(); // explicitly optional via Required(false)
    ...
    public void MissingFile_RequiredRule_Degrades()
    {
        ...
        var rule = Rule.From.File(path).Required().For<object>().Build();
        using var manager = new ConfigManager(new[]{rule});
    ...
    public void ValidJsonFile_LoadsCorrectly()
    {
        ...
        var rule = Rule.From.File(file.FilePath).Required().For<AppConfig>().Build();
    ...
    public void NestedJsonStructure_MapsToHierarchy()
    {
        ...
        var rule = Rule.From.File(file.FilePath).Required().For<NestedConfig>().Build();
        using var manager = new ConfigManager(new[]{rule}).Initialize();
    ...
    public void FileWithSelect_ExtractsSection()
    {
        ...
        var rule = Rule.From.File(file.FilePath).Select("App").Required().For<AppConfig>().Build();
        using var manager = new ConfigManager(new[]{rule}).Initialize();
