...
public sealed class X509HybridCrypto
{
    ...
    public HybridSecretEnvelope Encrypt(ReadOnlySpan<byte> plaintext)
    {
        ...
        // Heap copy of DEK needed for RSA API (accepts byte[], not Span)
        var dekArray = dek.ToArray();
        try
        {
            ...
            var wrappedKey = _rsa.Encrypt(dekArray, RSAEncryptionPadding.OaepSHA256);
        ...
        {
            // Zero out DEK from both stack and heap
            CryptographicOperations.ZeroMemory(dek);
            CryptographicOperations.ZeroMemory(dekArray);
        }
    }
