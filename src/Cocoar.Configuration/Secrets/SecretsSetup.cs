    ...
    public SecretsBuilder AllowPlaintext(bool allow = true)
    {
        if (allow)
        {
            System.Diagnostics.Trace.TraceWarning(
                "Cocoar.Configuration: AllowPlaintext() is enabled — Secret<T> values will be deserialized " +
                "from plaintext JSON without encryption. This is acceptable for development and testing " +
                "but should not be used in production. Use encrypted envelopes for production deployments.");
        }
