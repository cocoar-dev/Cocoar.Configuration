...
internal sealed class BindingRegistry
{
    ...
    public bool TryGetConcreteType(Type interfaceType, out Type concreteType)
    {
        return _interfaceToConcreteMap.TryGetValue(interfaceType, out concreteType!);
    }
    ...
    {
        return _interfaceToConcreteMap.TryGetValue(requestedType, out var concreteType) 
            ? concreteType 
            : requestedType;
    }
    ...
    private void BuildMappingTable(IEnumerable<BindingSpec> bindings)
    {
        ...
        foreach (var binding in bindings)
        {
            foreach (var interfaceType in binding.BoundInterfaces)
            {
                ...
                if (_interfaceToConcreteMap.ContainsKey(interfaceType))
                {
                    var existingConcrete = _interfaceToConcreteMap[interfaceType];
